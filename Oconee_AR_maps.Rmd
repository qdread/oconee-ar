...
```{r}
# Take log of E.coli counts and then scale all predictors.
...
# ESBL_mod <- glmer(ESBL ~ temperature + pH + conductivity + turbidity + Ecoli_counts + season + (1|site), data = w_env_ar_model_data)
# CRE_mod <- glmer(CRE ~ temperature + pH + conductivity + turbidity + Ecoli_counts + season + (1|site), data = w_env_ar_model_data)
# Enter_mod <- glmer(Enterococcus_presence ~ temperature + pH + conductivity + turbidity + Ecoli_counts + season + (1|site), data = w_env_ar_model_data)
# Salm_mod <- glmer(Salmonella_presence ~ temperature + pH + conductivity + turbidity + Ecoli_counts + season + (1|site), data = w_env_ar_model_data)
# AREc_mod <- glmer(AR_Ecoli_presence ~ temperature + pH + conductivity + turbidity + Ecoli_counts + season + (1|site), data = w_env_ar_model_data)
```

At first, I thought about using `glmer()` to fit a generalized linear mixed model (GLMM) to each taxon separately. As you can see above I ultimately decided to comment those lines out and not run them. Those models are OK but have some drawbacks. First, they are all separate models and do not account for the relationships between the different AR taxa, nor do they account for the "multiple comparisons" issue where we are testing the same predictors over and over to see if they affect different response variables. Second, several of the models result in singular fits. This means the data do not permit a good estimation of the random effects, so the maximum likelihood algorithm just "gives up" and calls all the random effects (site effects) zero. This is not ideal.

A superior way to do this would be to fit a MGLMM (multivariate generalized linear mixed model) where we fit the presence/absence of all five AR taxa as the response variable, which will account for the interactions between them, as well as accounting for the multiple comparisons reasonably well. The classical or frequentist statistical tools we have available are not up to this task. Luckily we have a trick up our sleeve and we can use a Bayesian approach to set up this model with prior distributions on the different parameters and then sample from the posterior.

I am following [the suggestion in this forum post](https://discourse.mc-stan.org/t/multivariate-glm-in-brms/11930/5) to convert the data to long form and include the interactions between taxon and the other predictors. The random effects are site and taxon nested within site. After compilation, the model samples in about 30 seconds on my laptop and converges well.
...
plot_marginal_effect <- function(variable, n_x = 50, backtransform_log = FALSE) {
  x_range <- range(w_env_ar_long[[variable]], na.rm = TRUE)
  x_r_seq <- seq(x_range[1], x_range[2], length.out = n_x)
  x_r_seq_backtransformed <- scaled_scales[variable] * temp_r_seq + scaled_centers[variable]
  ...
  ggplot(marginal_summ, aes(x = x, y = q0.5, color = taxon, fill = taxon)) + 
    ...
    scale_x_continuous(name = variable, trans = x_scale_trans) +
    labs(y = 'modeled probability of presence')
}

plot_marginal_effect(variable = 'temperature')
plot_marginal_effect(variable = 'pH')
plot_marginal_effect(variable = 'turbidity', backtransform_log = TRUE)
plot_marginal_effect(variable = 'conductivity', backtransform_log = TRUE)
plot_marginal_effect(variable = 'Ecoli_counts', backtransform_log = TRUE)
```

Now to make inferences based on those trends we can eyeball from the plots, let's estimate the marginal trends for each combination of taxon and predictor variable. These are displayed in both plot and table form.
