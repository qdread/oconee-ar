...
```{r, echo = FALSE}
w_ar_pres_abs <- melt(w_env_ar, id.vars = w_id_cols, measure.vars = c('AR_Ecoli_presence', 'Salmonella_presence',                   'Enterococcus_presence', 'ESBL', 'CRE'), variable.name = 'taxon', value.name = 'present')
w_ar_pres_abs[, taxon := gsub('_presence', '', taxon)]
w_ar_pres_abs_total <- w_ar_pres_abs[, .N, by = .(season, taxon, present)]
w_ar_pres_abs_total[, present := factor(present, labels = c('absent', 'present'))]

ggplot(w_ar_pres_abs_total[!is.na(present)], aes(x = season, y = N, fill = present)) +
...
I am following [the suggestion in this forum post](https://discourse.mc-stan.org/t/multivariate-glm-in-brms/11930/5) to convert the data to long form and include taxon as another effect in the model which can interact with the other predictors. After compilation, the model samples in about 10 seconds on my laptop and converges well.
...
mv_ar_fit <- brm(
  bf(present ~  taxon + temperature + pH + conductivity + turbidity + Ecoli_counts + season + taxon:temperature + taxon:pH + taxon:conductivity + taxon:turbidity + taxon:Ecoli_counts + taxon:season + (1|site)), 
  data = w_env_ar_long, family = bernoulli,
  prior = c(
    prior(normal(0, 3), class = b)
  ),
  chains = 4, iter = 2000, warmup = 1000, seed = 7122,
