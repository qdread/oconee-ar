...
# Data analysis: AR presence versus environmental variables
...
table_emtrends <- function(variable) {
  trends <- emtrends(mv_ar_fit, ~ taxon, var = paste0(variable, '_scaled'))
...
FIXME: here add tables of results

FIXME: here add the categorical model if needed

# Data analysis: Copy numbers of antibiotic resistance genes

This is question 1L. I am going to use the conclusion made by the UGA statistician that the absolute and relative copy numbers basically have the same distribution so it does not matter which one you analyze. I will use the absolute copy numbers. Here are some plots to show this. First process the data.
...
Fit the model. I am trying out a hurdle gamma and hurdle lognormal model. For each distribution, one of the models has an additional component that allows the probability of exactly zero to vary as a function of all the random and fixed effects. 
...
FIXME: Add tables of output here

# Data analysis: antibiotic concentrations

This is question 1M. First let's prepare the data and take a look at the distributions of each of the antibiotic concentrations. Use a pseudo-log transformation on the x-axis so we can see the spread of values on a log scale but also see the zero values. They are all non-negative (positive values with zeros).
...
## 2g-j. Change in ARG concentrations in influent vs. effluent
...
```{r}
ww_arg_abs_fit <- brm(
  copy_num ~ type + season + gene + gene:type + season:type + (1|site) + (1|id:site),
   data = ww_arg_abs[!gene %in% 'total'], family = hurdle_gamma(link = 'log', link_shape = 'log', link_hu = 'logit'),
   prior = c(
    prior(normal(0, 3), class = b)
  ),
  chains = 4, iter = 5000, warmup = 4000, seed = 72722,
...
ww_arg_rlt_fit <- brm(
  ...
  chains = 4, iter = 5000, warmup = 4000, seed = 72723,
